...
void halUartInit(uint8 baudrate, uint8 options)
{
    ...
    UCA1CTL1 |= UCSWRST;                    // Keep USART1 in reset state

    UCA1CTL1 |= UCSSEL1;                    // Set clock source SMCLK
    UCA1CTL1 &= ~UCSSEL0;

    P3SEL |= BIT6;                          // P3.6 = USART1 TXD
    P3SEL |= BIT7;                          // P3.7 = USART1 RXD
    
    P3DIR |= BIT6;                          // changing direction of P3.6 for USART1 RXD
    P3DIR &= ~BIT7;                         // changing direction of P3.7 for USART1 TXD

    switch (baudrate) {
    case HAL_UART_BAUDRATE_4800:
        UCA1CTL1 &= ~UCSSEL1;                    // Set clock source ACLK
        UCA1CTL1 |= UCSSEL0;
        UCA1BR0 = 0x06;                     //  4800
        UCA1BR1 = 0x00;                     //  4800
    ...
    case HAL_UART_BAUDRATE_9600:
        UCA1BR0 = 0x41;                     // 8MHz 9600
        UCA1BR1 = 0x03;                     // 8MHz 9600
        break;
    ...
    case HAL_UART_BAUDRATE_38400:
        UCA1BR0 = 0xD0;                     // 8MHz 38400
        UCA1BR1 = 0x00;                     // 8MHz 38400
        break;

    case HAL_UART_BAUDRATE_57600:
        UCA1BR0 = 0x8A;                     // 8MHz 57600
        UCA1BR1 = 0x00;                     // 8MHz 57600
        break;
    ...
    UCA1CTL1 &= ~UCSWRST;                   // Initialize USART1 state machine
...
* @return  none
*/
HAL_ISR_FUNCTION(usciA1Rx_ISR,USCIAB1RX)
{
...
* @return  none
*/
HAL_ISR_FUNCTION(usciB1Tx_ISR,USCIAB1TX)
{
    uint8 c;
