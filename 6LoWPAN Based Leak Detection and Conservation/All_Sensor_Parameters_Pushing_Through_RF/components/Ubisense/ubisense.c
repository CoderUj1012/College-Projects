...
/*	ret = p_tsl2561->I2C_BUS_READ_FUNC(p_tsl2561->dev_addr, command, &data, 1, UBI_YES);
	...
	p_tsl2561->gain = TSL2561_GAIN_0X; /* Setting default value. If you want to pass at time of init comment this line */
	p_tsl2561->integration = TSL2561_INTEGRATIONTIME_402MS;/* Setting default value. If you want to pass at time of init comment this line */
...
int tsl2561_get_raw_light_intensity(uint8 *raw_data)
{
	uint8 command,ret;
/*
...
uint32 tsl2561_get_lux_light_intensity(uint8 *raw_data)
{
	uint16 ch_0=0,ch_1=0;
	//	float ch_div=0;
...
uint16 sht21_get_raw_humidity(void)
{
	uint8 raw_data[2],command,ret;
	uint16 raw_humi;
...
uint16 sht21_get_raw_temperature(void)
{
	uint8 raw_data[2],command,ret;
        uint16 raw_temp;
...
float sht21_get_caliberated_humidity(uint16 raw_data)
{
	return (-6.0 + 125.0 / 65536.0 * (float)(raw_data));
}

float sht21_get_caliberated_temperature(uint16 raw_data)
{
...
uint16 tsl26711_get_proximity(void)
{
	uint8 data[2],command,ret;
	uint16 proximity = 0;
