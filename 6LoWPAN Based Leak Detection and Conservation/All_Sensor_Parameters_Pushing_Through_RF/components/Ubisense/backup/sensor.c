...
uint8 findSensors(uint8 slaveAddr){
  ...
  return Ack;
	/*P3SEL |= BIT1 + BIT2;                     // Assign I2C pins to USCI_B0 P3.1=SIMO SDA P3.2=SOMI SCL	
	UCB0CTL1 |= UCSWRST;                      // Enable SW reset.After this only we can configure the registers
	UCB0CTL0 = UCMST + UCMODE_3 + UCSYNC;     // I2C Master, synchronous mode
	UCB0CTL1 = UCSSEL_2 + UCSWRST;            // Use SMCLK, keep SW reset
	UCB0BR0 = 12;                             // fSCL = SMCLK/12 = ~100kHz   2400
	UCB0BR1 = 0;
        UCB0I2CSA = slaveAddr;                         // Slave Address is 069h
	...
	if(Ack)          			 // UCNACKIFG = 1 if not present , 0 if an ACK was received
         {                                       //__enable_interrupt();
						 //0 Acknowledge normally , 1 Generate NACK
        	UCB0CTL1 |= UCTXSTP;
	 }
