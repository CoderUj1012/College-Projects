...
static regVal_t regval[]= {
...
#ifdef SNIFFER_MODE
    //{CC2520_FRMFILT0,    0x0C}, 
    //{CC2520_FRMFILT1,    0x78},
    //{CC2520_MDMTEST1,    0x08},
#endif
   ...
   // {CC2520_GPIOCTRL5,   CC2520_GPIO_SNIFFER_CLK},
    //{CC2520_GPIOCTRL0,   0x2A},
    //{CC2520_GPIOCTRL1,   0x27},
    //{CC2520_GPIOCTRL2,   0x01},
    //{CC2520_GPIOCTRL3,   0x2D},
    //{CC2520_GPIOCTRL4,   0x2D},
    {CC2520_GPIOCTRL3,   0x2D},
    {CC2520_GPIOCTRL4,   0x2A},
    //{CC2520_GPIOCTRL5,   0x2C},
#endif
...
uint8 halRfInit(void)
{
    ...
    //halDigioIntDisable(&pinRadio_GPIO0);      //commented to avoid using gpio0
...
void halRfGetExtdAddr(uint8 *extdAddr){

  CC2520_MEMRD64(CC2520_RAM_EXTADDR, extdAddr);
}
...
* @fn      halRfSetExtdAddr
*
* @brief   Write extended address to chip
*
* @param   none
*
* @return  none
*/
void halRfSetExtdAddr(uint8 *extdAddr)
{
    CC2520_MEMWR64(CC2520_RAM_EXTADDR, extdAddr);
}
...
uint8 halRfTransmit(void)
{
    ...
    while(!CC2520_RSSI_VALID_PIN);      //changed to gpio5
    ...
    while(--timeout > 0) {
        ...
        HAL_INT_ON();
        if (CC2520_SAMPLED_CCA_PIN) break;      //changed to gpio3
          halMcuWaitUs(20);
    }
    if (timeout == 0) {
        status = FAILED;
    ...
    // Reconfigure GPIO2
    HAL_INT_OFF();
    CC2520_CFG_GPIO_OUT(0,     CC2520_GPIO_SFD);
    HAL_INT_ON();
    return status;
}
...
void halRfDisableRxInterrupt(void)
{
    ...
    halDigioIntClear(&pinRadio_GPIO2);
    halDigioIntDisable(&pinRadio_GPIO2);
}
...
{
    halDigioIntEnable(&pinRadio_GPIO2);
}
...
{
    halDigioIntSetEdge(&pinRadio_GPIO2, HAL_DIGIO_INT_RISING_EDGE);
    halDigioIntConnect(&pinRadio_GPIO2, pfISR);
    halDigioIntEnable(&pinRadio_GPIO2);
