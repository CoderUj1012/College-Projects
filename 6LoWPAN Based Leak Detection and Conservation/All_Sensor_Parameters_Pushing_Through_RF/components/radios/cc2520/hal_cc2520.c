...
uint8 CC2520_MEMRD8(uint16 addr)
{
    ...
    CC2520_SPI_TX_REG = CC2520_INS_MEMRD | HI_UINT16(addr);
    while((UCB1STAT & UCBUSY) != 0);*/
    
    CC2520_SPI_TXRX(LO_UINT16(addr));
    /*while ((UC1IFG & UCB1TXIFG) == 0);
    CC2520_SPI_TX_REG = LO_UINT16(addr);
    while((UCB1STAT & UCBUSY) != 0);*/
...
* @return  uint16 - result
*/
void CC2520_MEMRD64(uint16 addr, uint8 *data)
{
    uint8 i;
    CC2520_SPI_BEGIN();
    CC2520_SPI_TXRX(CC2520_INS_MEMRD | HI_UINT16(addr));
...
uint8 CC2520_MEMWR8(uint16 addr, uint8 value)
{
    ...
    s = CC2520_SPI_TXRX(CC2520_INS_MEMWR | HI_UINT16(addr));
    /*while ((UC1IFG & UCB1TXIFG) == 0);
    CC2520_SPI_TX_REG = CC2520_INS_MEMWR | HI_UINT16(addr);
    while((UCB1STAT & UCBUSY) != 0);*/
...
* @return  uint8 - status byte
*/
uint8 CC2520_MEMWR64(uint16 addr, uint8 *value)
{
    uint8 s,i;
    CC2520_SPI_BEGIN();
    s = CC2520_SPI_TXRX(CC2520_INS_MEMWR | HI_UINT16(addr));
