...
static void basicRfRxFrmDoneIsr(void)
{
    ...
    uint8 authStatus=0;
    #endif
    CC2520_CLEAR_EXC(CC2520_EXC_FIFOP);
    //if((P1IE) & (P1IFG) & (1<<6)){
    ...
    // Read payload length.
      ...
      //UCA1TXBUF = pHdr->packetLength;            // Output character
    ...
    // Only ack packets may be 5 bytes in total.
        ...
        #else
          halRfReadRxBuf(&rxMpdu[1], pHdr->packetLength);
          //halUartWrite(&rxMpdu[1], pHdr->packetLength - 2);
        #endif
...
void pc_based_gateway_rf_isr(void){
    basicRfPktHdr_t *pHdr;
    uint8 *pStatusWord,i;
    CC2520_CLEAR_EXC(CC2520_EXC_FIFOP);
...
uint8 pc_based_gate_way_sniffer(uint8 * buff){
	...
	// Read payload length.
      	halRfReadRxBuf(buff,1);

	//val = CC2520_MEMRD8(CC2520_EXCFLAG1);
	
	if((buff[0] > 126) || (buff[0] < 0)){
		halIntOff();
      		halRfEnableRxInterrupt();
		halIntOff();
		rxi.isReady = FALSE;
	...
	halRfReadRxBuf(&(buff[1]), buff[0]);
	CC2520_INS_STROBE(CC2520_STR_SFLUSHRX);
	halIntOff();
      	halRfEnableRxInterrupt();
	halIntOff();
	rxi.isReady = FALSE;
...
uint8 basicRfInit(basicRfCfg_t* pRfConfig)
{
  if (halRfInit()==FAILED){
    ...
    halRfSetExtdAddr(pConfig->extdAddr);
    ...
    halRfRxInterruptConfig(pc_based_gateway_rf_isr);
    halIntOn();
